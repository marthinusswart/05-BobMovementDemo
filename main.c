...
INCBIN_CHIP(packman_tiles, "packman_tiles.bpl")
...
int main() {
	...
	debug_register_bitmap(packman_tiles, "packman_tiles.bpl", 320, 320, 5, debug_resource_bitmap_interleaved | debug_resource_bitmap_masked);
	debug_register_palette(colors, "packman_tiles.pal", 32, 0);
	...
	*copPtr++ = 0x7fff;
	...
	while(!MouseLeft()) {
		...
		// C is 0-indexed. If you want the 5th Row and 1st Column (1-indexed):
		// Row 5 is index 4, Col 1 is index 0.
		int tileRow = 4;
		int tileCol = 0;
		int tileIndex = (tileRow * 20) + tileCol; 

		// A 16x16 Kingcon bob (5 planes + mask) takes exactly 320 bytes per tile.
		UBYTE *src = (UBYTE *)packman_tiles + (tileIndex * 320);

		WaitBlit();
		// Standard cookie-cutter: (A AND B) OR (C AND (NOT B)) -> 0xFA
		// The old code used 0xCA, which has a different, unusual effect.
		custom->bltcon0 = 0xfa | SRCA | SRCB | SRCC | DEST | ((x & 15) << ASHIFTSHIFT); // A = source, B = mask, C = background, D = destination
		custom->bltcon1 = ((x & 15) << BSHIFTSHIFT);
		custom->bltapt = src;
		custom->bltamod = 16 / 8;          // 2 bytes: skip mask to next plane's image data
		custom->bltbpt = src + 16 / 8 * 1; // 2 bytes: mask is immediately after image data
		custom->bltbmod = 16 / 8;          // 2 bytes: skip image to next plane's mask data

		// Note: x is rounded down to the nearest 16 pixel multiple before calculating 
		// the byte offset to ensure the Blitter pointer is word-aligned (an even address)
		custom->bltcpt = custom->bltdpt = (UBYTE *)image + (320 / 8) * 5 * y + (x / 16) * 2;
		custom->bltcmod = custom->bltdmod = (320 - 16) / 8; // 38 bytes
